...
//#define  CCA_RSSI_THRESH   0x80


/** SI4463硬件接口IO定义 */
#define SI4463_SDN_PORT				GPIOA
#define SI4463_SDN_CLK				RCC_APB2Periph_GPIOA
#define SI4463_SDN_PIN				GPIO_Pin_3

#define SI4463_IRQ_PORT				GPIOA
#define SI4463_IRQ_CLK				RCC_APB2Periph_GPIOA
#define SI4463_IRQ_PIN				GPIO_Pin_2

#define SI4463_GPIO0_PORT			GPIOB
#define SI4463_GPIO0_CLK			RCC_APB2Periph_GPIOB
#define SI4463_GPIO0_PIN			GPIO_Pin_6

#define SI4463_GPIO1_PORT			GPIOB
#define SI4463_GPIO1_CLK			RCC_APB2Periph_GPIOB
#define SI4463_GPIO1_PIN			GPIO_Pin_7

#define SI4463_GPIO2_PORT			GPIOA
#define SI4463_GPIO2_CLK			RCC_APB2Periph_GPIOB
#define SI4463_GPIO2_PIN			GPIO_Pin_0

#define SI4463_GPIO3_PORT			GPIOA
#define SI4463_GPIO3_CLK			RCC_APB2Periph_GPIOB
#define SI4463_GPIO3_PIN			GPIO_Pin_1
...
#define FRAC1		0x00
#define FRAC0		0x00
#define VCO_CNT0	0x07
#define VCO_CNT1	0x9F

extern short rssi_thresh_average;
...
public:
    ...
    void read_IRQ(void);
    void si446x_recalibration(void);

    void SI446x_Get_Modem_Status(uint8_t *pRead );
    void SI446x_Fast_RX_Hop( void );
    void SI446x_PH_Interrupt_Status( uint8_t *pRead );
    void SI446x_Get_FastA_REG(uint8_t *pRead );
    void SI446x_Set_Wut( uint8_t *pWrite );
    uint8_t si446x_get_rssi(void);
    uint8_t listen_before_transfer(uint8_t rssi_threshold);
//	unsigned short crc16(const void *data, unsigned short sizeInByte);
    void set_bothway_device_wut(void);
};

extern SI446X si4463;
...
typedef struct {
    uint8_t it_status[9];
    uint8_t modem_status[9];
    uint8_t rxtx_status;//ready 2 rx 0 tx 1
    uint8_t data_packet_len; //数据包长度
    uint8_t receive_len;	//收到数据长度
    uint8_t tx_fifo[txrx_max];
    uint8_t rx_fifo[txrx_max];
} WIRELESS_DATA_TXRX;

extern WIRELESS_DATA_TXRX si4463_txrx;
